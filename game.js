(function() {
    ...
    const cardBoki = document.getElementById('cardBoki');
    const cardNikki = document.getElementById('cardNikki');
    ...
    function init() {
        ...
        resize();
        loadBirdImage(characters[selectedCharacter].file);
        highScore = localStorage.getItem(characters[selectedCharacter].highScoreKey) || 0;
        initBackground();
        setupEventListeners();
    ...
    function setupEventListeners() {
        ...
        // Character selection
        cardBoki.addEventListener('click', () => selectCharacter('boki'));
        cardNikki.addEventListener('click', () => selectCharacter('nikki'));
    }

    function selectCharacter(name) {
        selectedCharacter = name;
        cardBoki.classList.toggle('active', name === 'boki');
        cardNikki.classList.toggle('active', name === 'nikki');
        highScore = localStorage.getItem(characters[name].highScoreKey) || 0;
        loadBirdImage(characters[name].file);
    }
    ...
    function restartGame() {
        gameOverScreen.classList.remove('visible');
        gameOverScreen.style.display = 'none';
        document.querySelector('.best-score').classList.remove('new-best');
        startGame();
    }
    ...
    function update(delta = 1) {
        ...
        // Pipes
        pipeTimer += delta;
        let pipeInterval = Math.max(80, Math.floor(window.innerWidth * 0.55 / gameSpeed));
        ...
        for (let i = pipes.length - 1; i >= 0; i--) {
            ...
            // Vertical pipe movement after 10 points
            if (pipes[i].vy) {
                pipes[i].top += pipes[i].vy * delta;
                pipes[i].bottom = pipes[i].top + pipeGap;

                // Bounce off top and bottom bounds
                if (pipes[i].top <= pipes[i].minY || pipes[i].bottom >= window.innerHeight - 100) {
                    pipes[i].vy *= -1;
                }
            }
    ...
    function addPipe() {
        ...
        // After 10 points, pipes move vertically
        let vy = 0;
        if (score >= 10) {
            vy = (Math.random() * 2 + 1) * (Math.random() > 0.5 ? 1 : -1);
        }

        pipes.push({ x: window.innerWidth, top: y, bottom: y + pipeGap, passed: false, vy, minY, maxY });
    }
    ...
    function gameOver() {
        ...
        if (isNewBest) {
            highScore = score;
            localStorage.setItem(characters[selectedCharacter].highScoreKey, highScore);
        }
        ...
        setTimeout(() => {
            ...
            if (isNewBest) {
                ...
                document.querySelector('.best-score').classList.add('new-best');
            } else {
                document.querySelector('.best-score').classList.remove('new-best');
            }
        }, 600);
    ...
    function triggerNewBestAnimation() {
        // Add NEW BEST badge
        const existing = document.getElementById('newBestBadge');
        if (existing) existing.remove();

        const badge = document.createElement('div');
        badge.id = 'newBestBadge';
        badge.innerHTML = '🏆 NEW BEST! 🏆';
        badge.style.cssText = `
            position: fixed;
            top: 50%;
            left: 50%;
            ...
            font-size: 22px;
            color: #FFD700;
            text-shadow: 3px 3px 0 #b7540a, 0 0 30px rgba(255,215,0,0.8);
            z-index: 999;
            white-space: nowrap;
            animation: newBestPop 2.5s ease forwards;
            pointer-events: none;
        `;
        document.body.appendChild(badge);

        // Confetti burst on canvas
        for (let i = 0; i < 80; i++) {
            setTimeout(() => {
                const colors = ['#FFD700', '#FF6B6B', '#74b9ff', '#a29bfe', '#55efc4', '#fd79a8', '#fff'];
                particles.push({
                    x: window.innerWidth / 2 + (Math.random() - 0.5) * window.innerWidth * 0.6,
                    y: window.innerHeight / 2 + (Math.random() - 0.5) * window.innerHeight * 0.3,
